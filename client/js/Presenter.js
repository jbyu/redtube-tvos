...
var Presenter = {
    ...
    load: function(event) {
        var self = this,
            ele = event.target,
            video = ele.getAttribute("video"),
            request = ele.getAttribute("request"),
...
function queryVideoURL(video) {
    var xhr = new XMLHttpRequest();
    xhr.onload = function () {
        var regex = /<source src="(.*)" type="video\/mp4">/;
        var match;
        var xml;
        if (match = regex.exec(this.responseText)) {
            console.log(match);
            startPlayback(match[1]);
        }
    }
    var url = "http://www.redtube.com" + video;
